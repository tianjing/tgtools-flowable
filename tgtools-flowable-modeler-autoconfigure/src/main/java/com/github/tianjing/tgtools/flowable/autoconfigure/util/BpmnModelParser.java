package com.github.tianjing.tgtools.flowable.autoconfigure.util;
...
public class BpmnModelParser {
    ...
    public static BpmnModelParser getInstance(BpmnModel p_Model) throws FlowableException {
        if (null == p_Model) {
            throw new FlowableException("无效的Model");
        }
    ...
    public static BpmnModelParser getInstance(String p_FlowID) throws FlowableException {
        if (StringUtils.isEmpty(p_FlowID)) {
            throw new FlowableException("无效的流程ID");
        }
    ...
    public String getPreActID(String p_TaskID) {
        Task task = ProcessEngines.getDefaultProcessEngine().getTaskService().createTaskQuery().taskId(p_TaskID).singleResult();
    ...
    public FlowElement getStartElement() {
        for (int i = 0, count = m_FlowElements.size(); i < count; i++) {
            if (m_FlowElements.get(i) instanceof StartEvent) {
                return m_FlowElements.get(i);
            }
        }
        return null;
    ...
    public UserTask getFirstTask() {
        ...
        for (int i = 0, count = eles.size(); i < count; i++) {
            if (eles.get(i) instanceof UserTask) {
                return (UserTask) eles.get(i);
            }
        }
        return null;
    }
     ...
     */
    public List<String> getCandidateUsersByFirstTask() throws FlowableException {
        UserTask task = getFirstTask();
        if (null != task) {
     ...
     * @return
     */
    public List<String> getCandidateUsers(String p_TaskDefID) throws FlowableException {
        UserTask task = getUserTask(p_TaskDefID);
        if (null == task) {
            throw new FlowableException("找不到UserTask,请检查流程图中的TaskDefID:" + p_TaskDefID);
        }
    ...
    private void parse() throws FlowableException {
        try {
            ...
            for (int i = 0, count = list.size(); i < count; i++) {
                ...
                while (flows.hasNext()) {
                    ...
                    if (artifact instanceof StartEvent) {


                        m_StartEvent = (StartEvent) artifact;

                    } else if (artifact instanceof UserTask) {
                        UserTask converter = (UserTask) artifact;
                    ...
                    else if (artifact instanceof EndEvent) {
                        m_EndEvent = (EndEvent) artifact;
                    }
                }
            ...
            throw new FlowableException("错误的模型，无法解析", var12);
        }
