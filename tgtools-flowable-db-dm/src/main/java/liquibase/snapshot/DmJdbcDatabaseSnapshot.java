...
/**
 * 主要是修正oracle sql  和 dm7 不兼容的部分
 * @author 田径
 * @create 2019-07-29 20:50
 * @desc
 **/
public class DmJdbcDatabaseSnapshot extends JdbcDatabaseSnapshot {
    ...
    public CachingDatabaseMetaData getMetaDataFromCache() throws SQLException {
        if (cachingDatabaseMetaData == null) {
            ...
            cachingDatabaseMetaData = new DmCachingDatabaseMetaData(this.getDatabase(), databaseMetaData);
        }
        ...
        public List<CachedRow> getUniqueConstraints(final String catalogName, final String schemaName, final String tableName) throws  DatabaseException {
            return getResultSetCache("getUniqueConstraints").get(new ResultSetCache.SingleResultSetExtractor(database) {
                ...
                public List<CachedRow> fastFetchQuery() throws SQLException, DatabaseException {
                    ...
                    return executeAndExtract(createSql(((AbstractJdbcDatabase) database).getJdbcCatalogName(catalogAndSchema), ((AbstractJdbcDatabase) database).getJdbcSchemaName(catalogAndSchema), tableName), DmJdbcDatabaseSnapshot.this.getDatabase(), (database instanceof InformixDatabase));
                }
                ...
                public List<CachedRow> bulkFetchQuery() throws SQLException, DatabaseException {
                    ...
                    return executeAndExtract(createSql(((AbstractJdbcDatabase) database).getJdbcCatalogName(catalogAndSchema), ((AbstractJdbcDatabase) database).getJdbcSchemaName(catalogAndSchema), null), DmJdbcDatabaseSnapshot.this.getDatabase());
                }
    ...
    private String getAllCatalogsStringScratchData() {
        return (String) DmJdbcDatabaseSnapshot.this.getScratchData(ALL_CATALOGS_STRING_SCRATCH_KEY);
    }
}
